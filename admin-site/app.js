...
function setupRoleBasedUI() {
    ...
    if (statusButtons && isPeer) {
        statusButtons.innerHTML = `
            ...
            <button class="status-btn busy" onclick="updateMyStatus('limited')">
                <i class="fas fa-clock"></i> Limited
            </button>
            <button class="status-btn off" onclick="updateMyStatus('unavailable')">
                <i class="fas fa-moon"></i> Unavailable
            </button>
        `;
    }
    ...
    container.innerHTML = activeAlerts.map(alert => `
        <div class="alert-card">
            <div class="alert-card-header">
                <span class="alert-name">${alert.user_name || 'Peer Supporter'}</span>
            ...
            <div class="alert-message">${alert.message || 'Peer needs immediate assistance'}</div>
            <div class="alert-time">${new Date(alert.created_at).toLocaleString()}</div>
            ${alert.acknowledged_name ? `<div class="alert-acknowledged-by">Acknowledged by: ${alert.acknowledged_name}</div>` : ''}
...
function renderCallbacks(callbacks) {
    ...
    // For counsellors, filter to counsellor-type callbacks
    // For peers, filter to peer-type callbacks
    ...
    // Render my active callbacks
    if (myContainer) {
        if (myCallbacks.length === 0) {
            myContainer.innerHTML = `
                <div class="empty-state">
                    <i class="fas fa-inbox"></i>
                    <p>No active callbacks</p>
                </div>
            `;
        } else {
            myContainer.innerHTML = myCallbacks.map(cb => `
                <div class="callback-card">
                    <div class="callback-header">
                        <span class="callback-name">${cb.name}</span>
                        <span class="callback-status in_progress">In Progress</span>
                    </div>
                    <div class="callback-phone"><i class="fas fa-phone"></i> ${cb.phone}</div>
                    <div class="callback-message">${cb.message || 'No message'}</div>
                    <div class="callback-actions">
                        <button class="btn btn-success" onclick="completeCallback('${cb.id}')">
                            <i class="fas fa-check"></i> Complete
                        </button>
                        <button class="btn btn-secondary" onclick="releaseCallback('${cb.id}')">
    ...
    if (pendingContainer) {
        ...
        if (pendingCallbacks.length === 0) {
            ...
            pendingContainer.innerHTML = pendingCallbacks.map(cb => `
                <div class="callback-card">
                    <div class="callback-header">
                        <span class="callback-name">${cb.name}</span>
                        <span class="callback-status pending">Pending</span>
                    </div>
                    <div class="callback-phone"><i class="fas fa-phone"></i> ${cb.phone}</div>
                    <div class="callback-message">${cb.message || 'No message'}</div>
                    <div class="callback-time">${new Date(cb.created_at).toLocaleString()}</div>
                    <div class="callback-actions">
                        <button class="btn btn-primary" onclick="takeCallback('${cb.id}')">
                            <i class="fas fa-hand-paper"></i> Take This Callback
                        </button>
                    </div>
                </div>
            `).join('');
...
async function triggerPanicAlert() {
    ...
    try {
        // Get peer profile to include contact info
        const peers = await apiCall('/peer-supporters');
        const myPeer = peers.find(p => p.user_id === currentUser.id);
