...
async function loadSettings() {
    try {
        ...
        const notificationEmailInput = document.getElementById('peer-notification-email');
        if (notificationEmailInput && siteSettings.peer_registration_notification_email) {
            notificationEmailInput.value = siteSettings.peer_registration_notification_email;
        }
    } catch (error) {
...
function setupRoleBasedUI() {
    ...
    var statusButtons = document.getElementById('my-status-buttons');
...
function renderPanicAlerts(alerts) {
    ...
    container.innerHTML = activeAlerts.map(function(alert) {
        ...
        html += '<div class="alert-card-header"><span class="alert-name">' + (alert.user_name || 'Peer Supporter') + '</span>';
        html += '<span class="alert-status ' + alert.status + '">' + alert.status + '</span></div>';
        if (alert.user_phone) {
            html += '<div class="alert-phone"><i class="fas fa-phone"></i> ' + alert.user_phone + '</div>';
        }
        html += '<div class="alert-message">' + (alert.message || 'Peer needs immediate assistance') + '</div>';
        html += '<div class="alert-time">' + new Date(alert.created_at).toLocaleString() + '</div>';
        if (alert.acknowledged_name) {
            html += '<div class="alert-acknowledged-by">Acknowledged by: ' + alert.acknowledged_name + '</div>';
        }
        html += '<div class="alert-actions">';
        if (alert.status === 'active') {
            html += '<button class="btn btn-warning" onclick="acknowledgeAlert(\'' + alert.id + '\')"><i class="fas fa-hand-paper"></i> Acknowledge</button>';
        }
        html += '<button class="btn btn-success" onclick="resolveAlert(\'' + alert.id + '\')"><i class="fas fa-check"></i> Resolve</button>';
        html += '</div></div>';
        return html;
    }).join('');
}
...
function renderCallbacks(callbacks) {
    ...
    var filteredCallbacks = callbacks;
    if (role === 'peer') {
        filteredCallbacks = callbacks.filter(function(c) { return c.request_type === 'peer'; });
    } else if (role === 'counsellor') {
        filteredCallbacks = callbacks.filter(function(c) { return c.request_type === 'counsellor'; });
    }
    
    var myCallbacks = filteredCallbacks.filter(function(c) { return c.assigned_to === userId && c.status === 'in_progress'; });
    var pendingCallbacks = filteredCallbacks.filter(function(c) { return c.status === 'pending'; });
    ...
    if (myContainer) {
        if (myCallbacks.length === 0) {
            ...
            myContainer.innerHTML = myCallbacks.map(function(cb) {
                return '<div class="callback-card"><div class="callback-header"><span class="callback-name">' + cb.name + '</span><span class="callback-status in_progress">In Progress</span></div><div class="callback-phone"><i class="fas fa-phone"></i> ' + cb.phone + '</div><div class="callback-message">' + (cb.message || 'No message') + '</div><div class="callback-actions"><button class="btn btn-success" onclick="completeCallback(\'' + cb.id + '\')"><i class="fas fa-check"></i> Complete</button><button class="btn btn-secondary" onclick="releaseCallback(\'' + cb.id + '\')"><i class="fas fa-undo"></i> Release</button></div></div>';
            }).join('');
        }
    }
    ...
    if (pendingContainer) {
        ...
        if (pendingCallbacks.length === 0) {
            ...
            pendingContainer.innerHTML = pendingCallbacks.map(function(cb) {
                return '<div class="callback-card"><div class="callback-header"><span class="callback-name">' + cb.name + '</span><span class="callback-status pending">Pending</span></div><div class="callback-phone"><i class="fas fa-phone"></i> ' + cb.phone + '</div><div class="callback-message">' + (cb.message || 'No message') + '</div><div class="callback-time">' + new Date(cb.created_at).toLocaleString() + '</div><div class="callback-actions"><button class="btn btn-primary" onclick="takeCallback(\'' + cb.id + '\')"><i class="fas fa-hand-paper"></i> Take This Callback</button></div></div>';
            }).join('');
        }
    }
}
...
async function updateMyStatus(newStatus) {
    try {
        ...
        if (role === 'counsellor') {
            ...
            var myCounsellor = counsellors.find(function(c) { return c.user_id === currentUser.id; });
            if (!myCounsellor) {
            ...
            var myPeer = peers.find(function(p) { return p.user_id === currentUser.id; });
            if (!myPeer) {
